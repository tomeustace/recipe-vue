...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.setRecipeData = setRecipeData;
	exports.createRecipe = createRecipe;

	var _recipe = __webpack_require__(1);

	var util = _interopRequireWildcard(_recipe);
	...
	function setRecipeData(recipe) {
	  console.log(recipe);
	  console.log(({"title":,"time":,"serves":,"ingredients":{},"methods":{"0":{"method":fds,"time":2}}}));
	  recipeData = recipe;
	}

	function createRecipe(elementId) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.addStepper = addStepper;
	exports.moveStep = moveStep;
	exports.getStepX = getStepX;
	exports.rescale = rescale;
	exports.getLineData = getLineData;

	var d3 = __webpack_require__(2);

	var container = exports.container = void 0;
	...
	var colors = ['#f44336', '#673ab7', '#009688', '#ffc107', '#607dbb'];

	var colorScale = exports.colorScale = d3.scaleLinear().range(colors).domain([0, 1, 2, 3, 4, 5]);

	var previous = '<svg xmlns="http://www.w3.org/2000/svg" fill="#000000" height="90" viewBox="0 0 40 40" width="90">\n                  <path stroke="green" d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/>\n                  <path d="M0 0h24v24H0z" fill="none"/>\n                </svg>';

	var next = '<svg xmlns="http://www.w3.org/2000/svg" fill="#000000" height="90" viewBox="0 0 40 40" width="90">\n                <path stroke="green" d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/>\n                <path d="M0 0h24v24H0z" fill="none"/>\n            </svg>';

	function initContainer(element) {
	  if (container === undefined) {
	    exports.container = container = d3.select('#' + element).append('svg').attr('width', width).attr('height', height).style('margin-top', '20px').append('g').attr('id', 'step-group');
	...
	function addStepper(step) {
	  var stepper = d3.select('.stepper');
	  var stepperPosition = 0;
	  ...
	  container.append('circle').attr('class', 'stepper').attr('cx', stepperPosition).attr('cy', '35').attr('r', radius / 2).attr('fill', 'green');
	}

	function moveStep(direction, target) {
	  var steps = d3.selectAll('.step');
	  var targetStep = '';
	  var currentPos = d3.select('.stepper').node().cx.baseVal.value;

	  if (direction === 'previous') {
	    (function () {

	      var prevs = steps.filter(function () {
	        return this.cx.baseVal.value < currentPos;
	      });

	      var max = d3.max(prevs.nodes(), function (circle) {
	      ...
	      d3.select('.stepper').attr('fill', 'blue').transition().duration(500).attr('fill', 'green').attr('cx', max);
	    })();
	  } else {
	    (function () {

	      var nexts = steps.filter(function () {
	        return this.cx.baseVal.value > currentPos;
	      });

	      var min = d3.min(nexts.nodes(), function (circle) {
	        return circle.cx.baseVal.value;
	      });
	      ...
	      d3.select('.stepper').attr('fill', 'blue').transition().duration(500).attr('fill', 'green').attr('cx', min);
	    })();
	  }

	  console.log(targetStep.datum().method);
	  d3.select('#recipe-method').select('span').transition().duration(500).text(targetStep.datum().method);
	}
	...
	function rescale() {
	  ...
	  var max = d3.max(steps.nodes(), function (circle) {
	    return circle.cx.baseVal.value;
	  });

	  if (max > 800) {
	    d3.select('#step-group').transition().duration(500).attr('transform', 'scale(0.7)');
	  }
	}
	...
	function addControlButtons() {
	  container.append('g').attr('class', 'previous').attr('transform', 'translate(0, ' + (height - 40) + ')').html(previous).style('cursor', 'pointer').on('click', function () {
	    moveStep('previous');
	  });

	  container.append('g').attr('class', 'next').attr('transform', 'translate(' + startOffset * 2 + ',' + (height - 40) + ')').html(next).style('cursor', 'pointer').on('click', function () {
	    moveStep('next');
	  });
	}

	function addMethodContainer(element) {
	  d3.select('#' + element).append('div').attr('id', 'recipe-method').attr('class', 'md-card').style('margin', '20px').style('padding', '10px').html('<span class="md-body-1"></span>');
	}
